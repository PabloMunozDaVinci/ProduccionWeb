...
class Producto extends ModeloPadre
{
    ...
    public static function paginate(Cnx $cnx, $pagina, $cuantos)
    {
        ...
        SELECT p.id, p.nombre, p.precio, p.categoria_id, p.path_original, p.path_editado, c.nombre nombre_categoria
        FROM productos p
    ...
    public function validate()
    {
        ...
        if (!$this->categoria_id) $errores['categoria_id'] = 'Ingresar una categoría';
        return $errores;
    }
    ...
    private function insert(Cnx $cnx)
    {
        $fecha = date('Y-m-d H:i:s');
        $consulta = $cnx->prepare('
            INSERT INTO productos(nombre, descripcion, categoria_id, precio, fecha_alta, fecha_modificacion)
            VALUES(:nombre, :descripcion, :categoria_id, :precio, :fecha_alta, :fecha_modificacion)
        ');
    ...
    {
        $consulta = $cnx->prepare('
            SELECT id, nombre, descripcion, precio, categoria_id, path_original, path_editado
            FROM productos
            WHERE id = :id
        ');
